...
[_validBuildings] spawn {
  ...
  while {tint_houses} do {
    ...
    _player = positionCameraToWorld [0,0,0];
    _buildings = (_player nearObjects ["Building", RANGE]) select {!(isObjectHidden _x) && {!(_x getVariable ["tint_house_blacklisted", false])} && {alive _x}};
